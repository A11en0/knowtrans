...
def get_eval_method(eval_method):
    ...
    if eval_method == 'likelihood':
        ...
        from AKB.evaluation import bandits
        return bandits.bandits_evaluator
    elif eval_method == 'f1_score':
        from AKB.evaluation import f1_score
        return f1_score.f1_evaluator
    else:
        raise ValueError('Invalid evaluation method.')


def evalute_prompts(task_type, prompts, eval_template, eval_data, demos_template, few_shot_data, eval_method, config):
    """
    ...
    return eval_method(task_type, prompts, eval_template, eval_data, demos_template, few_shot_data, config)
