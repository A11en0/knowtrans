...
#SBATCH -n 8
#SBATCH --gres=gpu:1
#SBATCH --output=./jellyfish-rag/logs/train-3
...
export WANDB_NAME=knowprep-7B-weight-r16
llamafactory-cli train jellyfish-rag/scripts/upstream-train-7B/rank/few-shot-merge-weight-r16.yaml
...
python jellyfish-rag/run_infer.py \
    ...
    --save_path "jellyfish-rag/results/7B/few-shot-train20-weight-r16" \
    --base_data_dir "data/Jellyfish-rag/test" \
    --model_weights "/share/home/12351018/pre-train/Jellyfish-7B" \
    --lora_weights "jellyfish-rag/outputs/7B/few-shot-train20-weight-r16" \
    --model_type "mistral" \
    --overwrite
