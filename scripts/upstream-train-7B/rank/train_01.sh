...
#SBATCH -n 8
#SBATCH --gres=gpu:1
#SBATCH --output=./jellyfish-rag/logs/01
...
llamafactory-cli train jellyfish-rag/scripts/upstream-train-7B/rank/few-shot-merge-weight-r128.yaml
...
python jellyfish-rag/run_infer.py \
    ...
    --save_path "jellyfish-rag/results/7B/few-shot-train20-weight-r128" \
    --base_data_dir "data/Jellyfish-rag/test" \
    --model_weights "/share/home/12351018/pre-train/Jellyfish-7B" \
    --lora_weights "jellyfish-rag/outputs/7B/few-shot-train20-weight-r128" \
    --model_type "mistral" \
    --overwrite    
