...
# from peft.tuners.lora.model import mark_only_lora_as_trainable
...
class MMOELoraLayer(LoraLayer):
    ...
    def __init__(self, base_layer: nn.Module, expert_num: int) -> None:
...
class MMOELoraLinear(nn.Module, MMOELoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        MMOELoraLayer.__init__(self, base_layer, self.expert_num)        
        
        # nn.Linear.__init__(self, in_features, out_features, **kwargs)        
        # MMOELoraLayer.__init__(self, expert_num=self.expert_num)
    ...
    def merge(self, task_ids):
        ...
        if self.r[self.active_adapter] > 0:
            expert_weight = self.lora_gate[self.active_adapter](self.lora_task_embedding[self.active_adapter](task_ids))
            for i in range(self.expert_num):
    ...
    def forward(self, x: torch.Tensor, task_ids, **kwargs):
        ...
        if self.disable_adapters:   # No adapter
            # if self.r[self.active_adapter] > 0 and self.merged: # merge the adapter to linear
                # self.unmerge(task_ids)
            # result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
            self.base_layer(x, **kwargs)
        else:
            for active_adapter in self.active_adapters:
                if active_adapter not in self.lora_A.keys():   # No adapter, directly use 
                    continue
                                            
                elif self.r[active_adapter] > 0 and not self.merged:   # general lora process
                    result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
                    
                    # x = x.to(self.lora_A[active_adapter].loraA[0].weight.dtype)
                    expert_weight = self.lora_gate[active_adapter](self.lora_task_embedding[active_adapter](task_ids))

                    for i in range(self.expert_num):
                        result = result + ( # lora process
    ...
    def _forward(self, x: torch.Tensor, task_ids, **kwargs):
        # task_ids = kwargs["task_ids"]
        previous_dtype = x.dtype
