import random
from AKB import generate, evaluate, config, template, data, llm
import re
...
def find_prompts(task_type,
    ...
    Parameters:
        ...
        seed_prompt: default prompt to compare with.
    Returns:
    ...
    if prompt_gen_template is None:
        ...
        prompt_gen_template = template.GenTemplate(prompt_gen_template, task_type)
    ...
    prompts = generate.generate_prompts(
        prompt_gen_template, demos_template, prompt_gen_data, conf['generation'], component)

    # remove extra [PROMPT] in prompt.
    # prompts = [prompt.replace('[PROMPT]','') for prompt in prompts]
    ...
    res = evaluate.evalute_prompts(task_type, prompts, eval_template, eval_data, demos_template, few_shot_data,
...
def refine_prompts(task, task_type, component, eval_template, demos_template,
                  error_data, eval_data, origin_prompt:template.Template, conf):
    eval_template = template.EvalTemplate(eval_template)
    demos_template = template.DemosTemplate(demos_template)
    transit_template = template.ErrorTemplate(
        task = task, 
        task_type = task_type, 
        component = component,
        task_description = origin_prompt.get_component("task_description"), 
        rules = origin_prompt.get_component("rules"), 
        question = origin_prompt.get_component("question"),
    )

    # only best prompt among last iteration is keeped by origin_prompt
    prompts = generate.refine_prompt(transit_template, error_data, conf['refine'])

    prompts.append(origin_prompt.get_component(component))
    print('Model returned {} prompts. Deduplicating...'.format(len(prompts)))
    prompts = list(set(prompts))
    print('Deduplicated to {} prompts.'.format(len(prompts)))
    for prompt in prompts:
    ...
    res = evaluate.evalute_prompts(task_type, prompts, eval_template, eval_data, demos_template, eval_data,
                                   conf['evaluation']['method'], conf['evaluation'])
...
def evaluate_prompts(task_type, prompts, eval_template, eval_data, demos_template, few_shot_data, conf,
                     base_conf='configs/default.yaml'):
    """
    ...
    eval_template = template.EvalTemplate(template=eval_template)
    demos_template = template.DemosTemplate(demos_template)

    print('Evaluating prompts...')
    res = evaluate.evalute_prompts(
        task_type, prompts, eval_template, eval_data, demos_template, few_shot_data, conf['evaluation']['method'],
        conf['evaluation'])
...
def get_evaluation_query(eval_template,
    ...
    if eval_query is None:
        if eval_base_method == 'likelihood':
            from AKB.evaluation import likelihood
